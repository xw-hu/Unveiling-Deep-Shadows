...
def zhxing_psnr(gt, pre, ignore_mask):
    ...
    if ignore_mask is not None:
        ...
        # Invert the mask to keep regions not ignored
        # print("Before: ", ignore_mask)
        ignore_mask = ignore_mask == 0
        # print("After: ", ignore_mask)

        # Apply the mask to filter out the "don't care" regions
        # print the summation of gt and pred
        gt = gt[ignore_mask]
        pre = pre[ignore_mask]
...
def evaluation(gt_root, pre_root, mask_root, demask_root, vis_flag):
    ...
    for fname in fnames:
        ...
        # psnr_non, ssim_non, rmse_non = metric(gt * (1 - mask), pre * (1 - mask), ignore_mask=ignore_mask)
        # psnr_shadow, ssim_shadow, rmse_shadow = metric(gt * mask, pre * mask, ignore_mask=ignore_mask)
    ...
    print('-----------------------------------------------------------------------------')
    # print(f'Shadow psnr: {round(np.average(psnr_shadow_list), 4)} ssim: {round(np.average(ssim_shadow_list), 4)}  rmse: {round(np.average(rmse_shadow_list), 4)}')
    # print(f'Non psnr: {round(np.average(psnr_non_list), 4)} ssim: {round(np.average(ssim_non_list), 4)} rmse: {round(np.average(rmse_non_list), 4)}')

##### evaluate all the methods at one time: get the folder names of all the methods under ./ShadowRemoval #####
# folder_names = os.listdir('/home/zhxing/Projects/ShadowSurvey/ShadowRemoval')
# folder_names.sort()
...
#     oupred_rootrs = '/home/zhxing/Projects/ShadowSurvey/ShadowRemoval/' + folder_name + '/SRD512_DESOBA'
#     evaluation(gt_root, pred_root, mask_root, demask_root, vis_flag=False)

##### evaluate one method at one time #####
mask_root = '/home/zhxing/Datasets/DESOBA_xvision/test/test_B_GT_NoSDDNet' # test_B_GT_NoSDDNet indicates the ground truth shadow mask here, not the one generated by SDDNet
gt_root = '/home/zhxing/Datasets/DESOBA_xvision/test/test_C'
input_root = '/home/zhxing/Datasets/DESOBA_xvision/test/test_A'
demask_root = '/home/zhxing/Datasets/DESOBA_xvision/InstanceMask'

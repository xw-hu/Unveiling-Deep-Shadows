...
3. **Stacked-CNN**: 
   ...
   - Hou et al., "Large scale shadow annotation and detection using lazy annotation and stacked CNNs", TPAMI, 2021. [[paper]](https://ieeexplore.ieee.org/document/9389981)  
...
5. **Patched-CNN**: Hosseinzadeh et al., "Fast shadow detection from a single image using a patched convolutional neural network", IROS, 2018. [[paper]](https://ieeexplore.ieee.org/document/8594156)  
...
7. **DSC**: 
   ...
   - Hu et al., "Direction-aware spatial context features for shadow detection and removal", TPAMI, 2020. [[paper]](https://ieeexplore.ieee.org/document/8995431)  
8. **DC-DSPF**: Wang et al., "Densely cascaded shadow detection network via deeply supervised parallel fusion", IJCAI, 2018. [[paper]](https://www.ijcai.org/Proceedings/2018/0634.pdf)  
9. **CPNet**: Mohajerani et al., "CPNet: A context preserver convolutional neural network for detecting shadows in single RGB images", MMSP, 2018. [[paper]](https://ieeexplore.ieee.org/document/8547112)  
10. **A+D Net**: Le et al., "A+D Net: Training a shadow detector with adversarial shadow attenuation", ECCV, 2018. [[paper]](https://link.springer.com/chapter/10.1007/978-3-030-01240-3_16)
...
14. **CPAdv-Net**: Mohajerani et al., "Shadow detection in single RGB images using a context preserver convolutional neural network trained by multiple adversarial examples", TIP, 2019. [[paper]](https://ieeexplore.ieee.org/document/8707976)  
...
17. **FDRNet**: Zhu et al., "Mitigating intensity bias in shadow detection via feature decomposition and reweighting", ICCV, 2021. [[paper]](https://openaccess.thecvf.com/content/ICCV2021/html/Zhu_Mitigating_Intensity_Bias_in_Shadow_Detection_via_Feature_Decomposition_and_Reweighting_ICCV_2021_paper.html)  
18. **FSDNet**: Hu et al., "Revisiting shadow detection: A new benchmark dataset for complex world", TIP, 2021. [[paper]](https://ieeexplore.ieee.org/document/9567582)  
19. **ECA**: Fang et al., "Robust shadow detection by exploring effective shadow contexts", ACM MM, 2021. [[paper]](https://dl.acm.org/doi/10.1145/3474085.3475454)  
20. **RCMPNet**: Liao et al., "Shadow detection via predicting the confidence maps of shadow detection methods", ACM MM, 2021. [[paper]](https://dl.acm.org/doi/10.1145/3474085.3475387)  
21. **SDCM**: Zhu et al., "Single image shadow detection via complementary mechanism", ACM MM, 2022. [[paper]](https://dl.acm.org/doi/10.1145/3503161.3547946)  
22. **TransShadow**: Jie et al., "A fast and efficient network for single image shadow detection", ICASSP, 2022. [[paper]](https://ieeexplore.ieee.org/document/9747395)  
...
26. **LRA&LDRA**: Yücel et al., "LRA&LDRA: Rethinking residual predictions for efficient shadow detection and removal", WACV, 2023. [[paper]](https://openaccess.thecvf.com/content/WACV2023/html/Yucel_LRA_LDRA_Rethinking_Residual_Predictions_for_Efficient_Shadow_Detection_WACV_2023_paper.html)  
27. **SDDNet**: Cong et al., "SDDNet: Style-guided dual-layer disentanglement network for shadow detection", ACM MM, 2023. [[paper]](https://dl.acm.org/doi/10.1145/3581783.3611762)  
28. **SDTR/SDTR+**: Wu et al., "How many annotations do we need for generalizing new-coming shadow images?", TCSVT, 2023. [[paper]](https://ieeexplore.ieee.org/document/10162324)  
29. **SAM-Adapter**: Chen et al., "SAM-Adapter: Adapting segment anything in underperformed scenes", ICCVW, 2023. [[paper]](https://openaccess.thecvf.com/content/ICCV2023W/html/Chen_SAM-Adapter_Adapting_Segment_Anything_in_Underperformed_Scenes_ICCVW_2023_paper.html)  
30. **ShadowSAM**: Chen et al., "Make segment anything model perfect on shadow detection", TGRS, 2023. [[paper]](https://ieeexplore.ieee.org/document/10223038)  
31. **AdapterShadow**: Jie et al., "AdapterShadow: Adapting segment anything model for shadow detection", arXiv, 2023. [[paper]](https://arxiv.org/abs/2307.15478)  
